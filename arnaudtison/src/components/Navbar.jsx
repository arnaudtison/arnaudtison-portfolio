  ...
  return (
    ...
    backgroundImage: scrollY < 50 && scrollY != null ? 'none' : 'url("data:image/svg+xml,%3Csvg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"%3E%3Cfilter id="noiseFilter"%3E%3CfeTurbulence type="fractalNoise" baseFrequency="0.8" numOctaves="3" stitchTiles="stitch"/%3E%3C/filter%3E%3Crect width="100%25" height="100%25" filter="url(%23noiseFilter)" opacity="0.2"/%3E%3C/svg%3E")'}} className="navigation-wrapper">
      <div className="logo-container">
        <img onClick={() => navigateToSection("home")} src={logo} alt="arnaudtison logo" />
      </div>
      <div className="redirect-links-container">
        <div onClick={() => navigateToSection("about")} className="redirect-link">WHO AM I?</div>
        <div onClick={() => navigateToSection("technologies")} className="redirect-link">TECHNOLOGIES</div>
        <div onClick={() => navigateToSection("work")} className="redirect-link">WORK</div>
        <div onClick={() => navigateToSection("contact")} className="redirect-link">GET IN TOUCH</div>
        <div className="external-links-container">
          <Instagram onClick={() => navigateToExternal("https://www.instagram.com/arnaudtison/")} size={35} />
          <Github onClick={() => navigateToExternal("https://github.com/arnaudtison")} size={35} />
        </div>
      </div>
    </div>
  );
